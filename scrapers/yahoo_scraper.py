...
def human_scroll(driver, scrolls=15):
    print("Executing DEEP human-like scrolling to trick Yahoo lazy-loading...")
    ...
    for _ in range(scrolls):
        ...
        if new_height == last_height:
            # The "Bounce" hack to trick Yahoo into loading more
...
def download_yahoo_images(driver, search_query, folder_name, max_images):
    ...
    search_url = f"https://images.search.yahoo.com/search/images?p={formatted_query}"
    ...
    if len(image_urls) < 10:
        print(f" WARNING: Yahoo didn't load enough images for '{search_query}'.")
        return 0 # Trigger strike counter

    print(f"Found {len(image_urls)} valid Yahoo targets. Extracting...")
    count = 0
        ...
        try:
            ...
            if response.status_code == 200:
                if len(response.content) > 3000: 
                    file_path = os.path.join(folder_path, f"img_yahoo_{count}.jpg")
...
def main():
    print("="*50)
    print(" MULTI-ENGINE IMAGE SCRAPER: YAHOO MODULE")
    print("="*50)
    ...
    for query, folder_name in targets:
        ...
        try:
            result = download_yahoo_images(driver, query, folder_name, MAX_IMAGES_PER_CLASS)
            
            if result == "SKIPPED":
                pass 
            elif result == 0 or result < 10:
                consecutive_fails += 1
                print(f" STRIKE {consecutive_fails}! Yahoo blocked this download.")
            else:
        ...
        if consecutive_fails >= MAX_STRIKES:
            print("\n FATAL ERROR: Yahoo Anti-Bot Tripwire Triggered! 🚨🚨")
    ...
    print(f"\n YAHOO SCRAPING RUN COMPLETE! Images safely stored in {MASTER_OUTPUT_DIR}/.")
