...
MIN_COOLDOWN = 12                      # Minimum seconds to wait between targets
MAX_COOLDOWN = 22                      # Maximum seconds to wait between targets
...
def setup_stealth_browser():
    ...
    # version_main removed for universal compatibility
    driver = uc.Chrome(options=options) 
    return driver

def human_scroll(driver, scrolls=15):
    print("Executing DEEP human-like scrolling to trick Bing lazy-loading...")
...
def download_bing_images(driver, search_query, folder_name, max_images):
    ...
    search_url = f"https://www.bing.com/images/search?q={formatted_query}"
    ...
    if len(image_urls) < 10:
        print(f" WARNING: Bing might be blocking us, or no relevant images found for '{search_query}'.")
        return 0 # Trigger strike counter

    print(f"Found {len(image_urls)} valid Bing targets. Extracting...")
    count = 0
    ...
    for url in image_urls:
        try:
            ...
            if response.status_code == 200:
                if len(response.content) > 3000: # Filter out tiny junk thumbnails
                    file_path = os.path.join(folder_path, f"img_bing_{count}.jpg")
...
def main():
    print("="*50)
    print("  MULTI-ENGINE IMAGE SCRAPER: BING MODULE")
    print("="*50)
    ...
    for query, folder_name in targets:
        ...
        try:
            result = download_bing_images(driver, query, folder_name, MAX_IMAGES_PER_CLASS)
            
            if result == "SKIPPED":
                pass 
            elif result == 0 or result < 10:
                consecutive_fails += 1
                print(f" STRIKE {consecutive_fails}! Bing blocked this download.")
            else:
        ...
        if consecutive_fails >= MAX_STRIKES:
            print("\n FATAL ERROR: Bing Anti-Bot Tripwire Triggered! 🚨🚨")
    ...
    print(f"\n BING SCRAPING RUN COMPLETE! Images safely stored in {MASTER_OUTPUT_DIR}/.")
